...
#include "src/fastertransformer/utils/gemm_test/bart_gemm_func.h"
...
template<typename T>
void generate_bart_gemm_config(int   batch_size,
                               int   beam_width,
                               int   max_mem_seq_len,
                               int   encoder_d_model,
                               ...
                               bool  is_fp16_compute_type)
{
    ...
    for (int i = 0; i < gemm_num; ++i) {
        ...
        if (data_type != FLOAT_DATATYPE && i != 1 && i != 2 && i != 0 && i != 10) {
            ...
            // for bart, computeType & scaleType should be FP32
            if (is_fp16_compute_type) {
                using scaleT       = typename ScaleTypeConverter<T, true>::Type;
    ...
    printf("***Bart Gemm Testing End***\n");
    return;
}

template void generate_bart_gemm_config<float>(int   batch_size,
                                               int   beam_width,
                                               int   max_mem_seq_len,
                                               int   encoder_d_model,
                                               int   encoder_head_num,
...
template void generate_bart_gemm_config<half>(int   batch_size,
                                              int   beam_width,
                                              int   max_mem_seq_len,
                                              int   encoder_d_model,
                                              int   encoder_head_num,
...
#ifdef ENABLE_BF16
template void generate_bart_gemm_config<__nv_bfloat16>(int   batch_size,
                                                       int   beam_width,
                                                       int   max_mem_seq_len,
                                                       int   encoder_d_model,
                                                       int   encoder_head_num,
...
size_t calBartGemmTestBufSizeInByte(int            batch_size,
                                    int            beam_width,
                                    int            max_mem_seq_len,
                                    int            encoder_d_model,
                                    int            encoder_head_num,
